...
const Footer = ({
  ...
  function Hours(time: string): JSX.Element {
    ...
    if (hour >= 12) {
      return (
        <>
          {hour - 12}
          {minuteStr} <span className="all-small-caps">PM</span>
        </>
      );
    } else {
      return (
        <>
          {hour}
          {minuteStr} <span className="all-small-caps">AM</span>
        </>
      );
    }
  }
  ...
  const google = featuredSocials.find(
    (social: { site?: { service?: string } }) =>
      social.site?.service === "google",
  );
    ...
    <footer>
      ...
      {showQuote && (
        <>
          ...
          >
            ...
            {addressLink && <p className="error">{strapiForm.addressLink}</p>}
            ...
            {(emailProfanity || messageProfanity || referralProfanity) && (
              <p className="error">{strapiForm.profanity}</p>
            )}
              ...
              <p>
                ...
                <small>
                  ...
                  <span className="white-space-no-wrap">
                    {Hours(strapiForm.opening)}&thinsp;and&thinsp;
                    {Hours(strapiForm.closing)}&thinsp;
                    <span className="all-small-caps">PST</span>
                  </span>
                  , &nbsp;
                  <span className="white-space-no-wrap">
                    {days(strapiForm.days)}
                  </span>
                  .&nbsp;
                </small>
              </p>
      ...
      <div id="team" className="stork">
        <h3 className="margin-block-end-vinson">
          <Link to="/team" className="link--subtle">
            Meet Our Team
          </Link>
        </h3>
        ...
        <div className="team-heads spin">
          {allStrapiTeam.nodes.map((team: TeamType) => (
            <Link key={team.slug} to={`/team/${team.slug}`}>
              <GatsbyImage
                image={
      ...
      <hr className="albatross" />
      {featuredSocials.length > 0 && <Socials services={featuredSocials} />}
      ...
      {starRating && (
        <p className="albatross text-align-center margin-block-start-denali">
          <a
            href={`${google?.site.link}${google?.username}`}
            target="_blank"
      ...
      <div className="footer-copyright">
        <h4 className="sr-only footer-copyright__mind-the-gap">
          <Link to="/">{strapiAbout.businessName}</Link>
        </h4>
        ...
        <button
          ...
          onClick={() =>
            window.dispatchEvent(new Event("sierra:show-cookie-preferences"))
          }
        >
          Cookie Settings
        </button>
